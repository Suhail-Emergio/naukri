    ...
    if await Subscription.objects.filter(user=request.auth, plan__resdex=True).aexists():
        ...
        if job_id:
            if await JobPosts.objects.filter(id=job_id).aexists():
                ...
                personal = await Personal.objects.filter(queries).values_list('user', flat=True).order_by('-id')
...
async def candidates_invited(request, job_id: int = None):
    ...
    for i in invites:
        ...
        candidates.append({"id": i.id, "candidate":{"personal": {"personal": personal, "user": user}, "employment": employment, "qualification": qualification}, "read": i.read, "job": job, "created_on": i.created_on})
    return 200, candidates
