...
async def resdex(request,
    ...
    if job_id is not None:
        if await JobPosts.objects.filter(id=job_id).aexists():
            job = await JobPosts.objects.aget(id=job_id)
            skills = await sync_to_async(lambda: job.skills or [])()
            keyword_query = Q()
            for keyword in skills:
                if keyword:
        ...
        elif experience_month is not None:
            queries &= Q(total_experience_months__gte=experience_month)
        ...
        if salary_min is not None and salary_max is not None:
            ...
            queries &= Q(prefered_salary_pa__gte=salary_min)
        elif salary_max is not None:
            queries &= Q(prefered_salary_pa__lte=salary_max)
...
async def recruiter_counts(request):
    if await Subscription.objects.filter(user=request.auth).aexists():
        ...
        active_jobs_count = await JobPosts.objects.filter(company__user=request.auth, active=True,status='active').acount()
