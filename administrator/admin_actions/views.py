...
@paginate
async def all_jobs(request, order: str = 'active', limit: int = 20, offset: int = 0):
    user = request.auth
...
# @paginate
def job_post_application(request, job_id: int, order: str = 'active', limit: int = 20, offset: int = 0):
    user = request.auth
    if user.is_superuser:
        applications = []
        views = 0
...
@paginate
def all_applications(request, order: str = 'active', limit: int = 20, offset: int = 0):
    user = request.auth
    if user.is_superuser:
        applications = []
        for i in ApplyJobs.objects.filter(job__id=job_id).order_by(f'-{order}'):
...
@paginate
def all_company(request, limit: int = 20, offset: int = 0):
...
@paginate
def all_seekers(request, limit: int = 20, offset: int = 0):
    seekers = []
...
@paginate
def all_subs(request, type: str = "all", limit: int = 20, offset: int = 0):
    seekers = []
