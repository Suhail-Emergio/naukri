...
def all_company(request):
    ...
    for i in CompanyDetails.objects.all():
        ...
        companies.append({"name": i.name, "about": i.about, "website": i.website, "functional_area": i.functional_area, "address": i.address, "state": i.state, "city": i.city, "postal_code": i.postal_code, "logo": i.logo, "id": i.id, "jobs": jobs})
    return companies
...
def edit_notifications(request, id: str, title: str = Form(None), description: str = Form(None), url: str = Form(None), image: UploadedFile = File(None)):
